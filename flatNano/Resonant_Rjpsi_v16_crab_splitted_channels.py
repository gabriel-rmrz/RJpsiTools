'''
Difference from v9:
-reference triggers for Sfs
- selection on the 2Mu3P channel similar to hadronic channel: doca, vprob, fls3d  
Difference from v8:
...
#channels = ['BTo2Mu','BTo3Mu','BTo2MuP','BTo2MuK','BTo2Mu3P']
...
for dataset in [args.data,args.mc_mu,args.mc_tau,args.mc_bc,args.mc_hb,args.mc_onia,args.mc_gen]: 
    ...
    for i,fname in enumerate(paths):
        ...
        for channel in channels:
            ...
            muons = nf['Muon']
            bcands = nf[channel]
            #for i, j in zip(bcands.mu1Idx, bcands.mu2Idx):
            #    if len(i)!=0 and channel=="BTo2Mu3P" :
            #        print(i,j) 
            hlt = nf['HLT']
            gen= nf['GenPart']
            bcands['hlt'] = nf['HLT']
            ...
            #trigger_selection =((bcands.mu1.isMuonFromJpsi_doubleMuTrg==True)  & (bcands.mu2.isMuonFromJpsi_doubleMuTrg==True))
            #trigger_selection = ((bcands.mu1.isDimuon0_jpsi_Trg==True)  & (bcands.mu2.isDimuon0_jpsi_Trg==True))
            if (channel=='BTo2MuP') or (channel=='BTo2MuK'):
                trigger_selection = ((bcands.mu1.isDimuon0_jpsi_Trg==True) & (bcands.mu2.isDimuon0_jpsi_Trg==True))
                #trigger_selection = ((bcands.mu1.isDoubleMuTrg==True) &(bcands.mu2.isDoubleMuTrg==True))
            if (channel=='BTo3Mu'):
                #trigger_selection = (bcands.mu1.isMu8_Trg==True) 
                #trigger_selection = ((bcands.mu1.isDoubleMuTrg==True) &(bcands.mu2.isDoubleMuTrg==True))
                trigger_selection = ((bcands.mu1.isDimuon0_jpsi_Trg==True) & (bcands.mu2.isDimuon0_jpsi_Trg==True))
            if (channel=='BTo2Mu'):
                trigger_selection = (bcands.mu1.isMu8_Trg==True)
                b_selection = ( (bcands.p4.mass>2)&(bcands.p4.mass<4)  ) 
            if(channel == 'BTo2Mu3P'):
                pions_selection = ((bcands.ip3D_pvjpsi_pi1 >-0.04) & (bcands.ip3D_pvjpsi_pi1 <0.06) & (bcands.ip3D_pvjpsi_pi2 >-0.04) & (bcands.ip3D_pvjpsi_pi2 <0.06) & (bcands.ip3D_pvjpsi_pi3 >-0.04) & (bcands.ip3D_pvjpsi_pi3 <0.06))
                tau_selection = ((bcands.tau_vprob >0.1) & (bcands.tau_fls3d >3))
                #trigger_selection =((bcands.mu1.isMuonFromJpsi_jpsiTrkTrg==True) & (bcands.mu2.isMuonFromJpsi_jpsiTrkTrg==True) & ((bcands.pi1Trg ==True) | (bcands.pi2Trg ==True) | (bcands.pi3Trg ==True)))
                trigger_selection = ((bcands.mu1.isDimuon0_jpsi_Trg==True) & (bcands.mu2.isDimuon0_jpsi_Trg==True))# HLT_Dimuon0_Jpsi_NoVertexing==True
                #trigger_selection = ((bcands.mu1.isDoubleMuTrg==True) &(bcands.mu2.isDoubleMuTrg==True))
                #trigger_selection = (bcands.mu1.isMu8_Trg==True)    
            ...
            if (dataset==args.mc_onia or dataset==args.mc_hb):
                if(channel == 'BTo2Mu3P'):
                    ...
                    x_selection= ~  (( bcands.mu1.genPartIdx>=0) & (bcands.mu2.genPartIdx>=0) & (abs(bcands.mu1.mother.pdgId) == 443) & (abs(bcands.mu2.mother.pdgId) == 443) & (abs(bcands.mu1.grandmother.pdgId) == 541) & (abs(bcands.mu2.grandmother.pdgId) == 541) )
            if channel == 'BTo2MuP':
           ...
           #     print(i.nMuon)
            ...
            for selection,name in zip(flag_selection, flag_names):
                ...
                for chan, tab, sel in [
                    ...
                    df['jpsivtx_maxdoca'] = tab.jpsivtx_maxdoca
                    df['jpsivtx_dR'] = tab.jpsivtx_dR
                    ...
                    if(channel == 'BTo3Mu'):
                        ...
                        ''' htl: These branches are not present in the ntuples
                    ...
                    df['HLT_DoubleMu4_LowMassNonResonantTrk_Displaced'] = tab.hlt.DoubleMu4_LowMassNonResonantTrk_Displaced
                    df['HLT_DoubleMu4_PsiPrimeTrk_Displaced'] = tab.hlt.DoubleMu4_PsiPrimeTrk_Displaced
                    df['HLT_DoubleMu4_3_Jpsi'] = tab.hlt.DoubleMu4_3_Jpsi
                    df['HLT_Dimuon0_Jpsi_NoVertexing'] = tab.hlt.Dimuon0_Jpsi_NoVertexing
                    df['HLT_DoubleMu4_3_Jpsi_Displaced'] = tab.hlt.DoubleMu4_3_Jpsi_Displaced
                    df['HLT_Mu8'] = tab.hlt.Mu8_v
                    df['HLT_Dimuon0_Jpsi_L1_4R_0er1p5R'] = tab.hlt.Dimuon0_Jpsi_L1_4R_0er1p5R
                    ...
                    df['mu1_isFromJpsi_DoubleMuT'] = tab.mu1.isMuonFromJpsi_doubleMuTrg
                    ...
                    df['mu1_isDoubleMuT'] = tab.mu1.isDoubleMuTrg
                    df['mu1_isDimuon0_jpsi_Trg'] = tab.mu1.isDimuon0_jpsi_Trg
                    df['mu1_isDimuon0_jpsi_4R_Trg'] = tab.mu1.isDimuon0_jpsi_4R_Trg
                    df['mu1_isDimuon43_jpsi_displaced_Trg'] = tab.mu1.isDimuon43_jpsi_displaced_Trg
                    df['mu1_isMu8_Trg'] = tab.mu1.isMu8_Trg
                    df['mu1_isMu8_Trg_HLTps'] = tab.mu1.isMu8_Trg_HLTps
                    df['mu1_isMu8_Trg_L1ps'] = tab.mu1.isMu8_Trg_L1ps
                    df['mu1_isDimuon0_jpsi_Trg_HLTps'] = tab.mu1.isDimuon0_jpsi_Trg_HLTps
                    df['mu1_isDimuon0_jpsi_Trg_L1ps'] = tab.mu1.isDimuon0_jpsi_Trg_L1ps
                    df['mu1_isDimuon43_jpsi_displaced_Trg_HLTps'] = tab.mu1.isDimuon43_jpsi_displaced_Trg_HLTps
                    df['mu1_isDimuon43_jpsi_displaced_Trg_L1ps'] = tab.mu1.isDimuon43_jpsi_displaced_Trg_L1ps
                    df['mu1_isFromDoubleMuT_HLTps']=tab.mu1.isDoubleMuTrg_HLTps
                    df['mu1_isFromDoubleMuT_L1ps']=tab.mu1.isDoubleMuTrg_L1ps
                    df['mu1_isDimuon0_jpsi_4R_Trg_HLTps'] = tab.mu1.isDimuon0_jpsi_4R_Trg_HLTps
                    df['mu1_isDimuon0_jpsi_4R_Trg_L1ps'] = tab.mu1.isDimuon0_jpsi_4R_Trg_L1ps
                    ...
                    df['mu2_isFromJpsi_DoubleMuT'] = tab.mu2.isMuonFromJpsi_doubleMuTrg
                    ...
                    df['mu2_isFromDoubleMuT'] = tab.mu2.isDoubleMuTrg
                    df['mu2_isDimuon0_jpsi_Trg'] = tab.mu2.isDimuon0_jpsi_Trg
                    df['mu2_isDimuon0_jpsi_4R_Trg'] = tab.mu2.isDimuon0_jpsi_4R_Trg         
                    df['mu2_isDimuon43_jpsi_displaced_Trg'] = tab.mu2.isDimuon43_jpsi_displaced_Trg
                    df['mu2_isMu8_Trg'] = tab.mu2.isMu8_Trg
                    df['mu2_isMu8_Trg_HLTps'] = tab.mu2.isMu8_Trg_HLTps
                    df['mu2_isMu8_Trg_L1ps'] = tab.mu2.isMu8_Trg_L1ps
                    df['mu2_isDimuon0_jpsi_Trg_HLTps'] = tab.mu2.isDimuon0_jpsi_Trg_HLTps
                    df['mu2_isDimuon0_jpsi_Trg_L1ps'] = tab.mu2.isDimuon0_jpsi_Trg_L1ps
                    df['mu2_isDimuon43_jpsi_displaced_Trg_HLTps'] = tab.mu2.isDimuon43_jpsi_displaced_Trg_HLTps
                    df['mu2_isDimuon43_jpsi_displaced_Trg_L1ps'] = tab.mu2.isDimuon43_jpsi_displaced_Trg_L1ps
                    df['mu2_isFromDoubleMuT_HLTps']=tab.mu2.isDoubleMuTrg_HLTps
                    df['mu2_isFromDoubleMuT_L1ps']=tab.mu2.isDoubleMuTrg_L1ps
                    df['mu2_isDimuon0_jpsi_4R_Trg_HLTps'] = tab.mu2.isDimuon0_jpsi_4R_Trg_HLTps
                    df['mu2_isDimuon0_jpsi_4R_Trg_L1ps'] = tab.mu2.isDimuon0_jpsi_4R_Trg_L1ps
                    ...
                    if(channel == 'BTo3Mu'):
                        ...
                        df['k_isFromJpsi_DoubleMuT'] = tab.k.isMuonFromJpsi_doubleMuTrg
                        ...
                        df['k_isFromDoubleMuT'] = tab.k.isDoubleMuTrg
                    ...
                    if (channel == 'BTo2Mu3P'):
                        df['tau_vprob'] = tab.tau_vprob 
                        df['tau_fls3d'] = tab.tau_fls3d
                        df['tau_fitted_pt'] = tab.tau_fitted_pt
                        df['pi1Trg']= tab.pi1Trg
                        df['pi2Trg']= tab.pi2Trg 
                        df['pi3Trg']= tab.pi3Trg 
                        df['ip3D_pvjpsi_pi1'] = tab.ip3D_pvjpsi_pi1
                        df['ip3D_pvjpsi_pi2'] = tab.ip3D_pvjpsi_pi2
                        df['ip3D_pvjpsi_pi3'] = tab.ip3D_pvjpsi_pi3
                        df['pi1_d0sig'] = tab.pi1_d0sig
                        df['pi2_d0sig'] = tab.pi2_d0sig
                        df['pi3_d0sig'] = tab.pi3_d0sig                               
                    
                        df['pi1_mumuTrk_chi2ndof'] = tab.mumuTrk_pi1_chi2/tab.mumuTrk_pi1_ndof
                        df['pi2_mumuTrk_chi2ndof'] = tab.mumuTrk_pi2_chi2/tab.mumuTrk_pi2_ndof
                        df['pi3_mumuTrk_chi2ndof'] = tab.mumuTrk_pi3_chi2/tab.mumuTrk_pi3_ndof                   

                    if (channel == 'BTo2MuP' or channel == 'BTo2MuK'):
                        df['k_d0sig'] = tab.k_d0sig
                    ...
                    if(channel == 'BTo2Mu3P'):
                        df['ip3d_pvjpsi_pi1'] = tab.ip3D_pvjpsi_pi1
                        df['ip3d_pvjpsi_pi1_e'] = tab.ip3D_pvjpsi_pi1_e
                        df['ip3d_pvjpsi_pi2'] = tab.ip3D_pvjpsi_pi2
                        df['ip3d_pvjpsi_pi2_e'] = tab.ip3D_pvjpsi_pi2_e
                        df['ip3d_pvjpsi_pi3'] = tab.ip3D_pvjpsi_pi3
                        df['ip3d_pvjpsi_pi3_e'] = tab.ip3D_pvjpsi_pi3_e

                    if(channel != 'BTo2Mu3P' and channel != 'BTo2Mu'):
                    ...
                    if(channel=='BTo3Mu'):
                        ...
                        final_dfs_2m[name] = pd.concat((final_dfs_2m[name], dfs[name])) 
    ...
    for flag in flag_names:
        for channel in channels:
            #if channel == 'BTo2Mu':
            #    final_dfs_2m[flag].to_root('REPLACE_FILE_OUT'+'_'+flag+adj+'.root', key=channel)
            if channel == 'BTo3Mu':
                ...
                final_dfs_2m[flag].to_root('REPLACE_FILE_OUT'+'_'+flag+adj+'.root', key=channel, mode = 'a')
            #elif channel == 'BTo3Mu':
            #    final_dfs_mmm[flag].to_root('REPLACE_FILE_OUT'+'_'+flag+adj+'.root', key=channel, mode = 'a')
